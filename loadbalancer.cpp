...
/**
 * @brief Add a number of requests to the request queue if the queue is not full.
...
void LoadBalancer::addRequests(int numRequests) {
    for (int i = 0; i < numRequests; i++) {
        if (requestQueue.size() < maxQueueSize) {
            ...
            printf("Max request queue size reached, rejecting request\n");
            requestLogs.push_back({Request(createIP(), createIP(), rand() % 10 + 1), 0, 0});
        }
    }
}
...
void LoadBalancer::setActiveServers() {
    ...
    if (numServersActive != reqsPerServer) {
        printf("Queue size: %ld, Active server count: %d\n", requestQueue.size(), reqsPerServer);
    }
